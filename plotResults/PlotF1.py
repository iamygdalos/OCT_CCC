...
def ReadALlFilesForCVRun(folder, CVRun):
    ...
    for fileName in os.listdir(folder + '\\' + CVRun + '\\'):
        ...
        plt.plot('x', 'y1', data=df_f, color='darkblue', linewidth=1.5, label='train f1-score')
        plt.plot('x', 'y2', data=df_f, color='crimson', linewidth=1.5, label='valid f1-score')
        ...
        plt.savefig(folder + '\\' + CVRun + '\\plots\\f1_score\\' + os.path.splitext(fileName)[0] + '_f1_score.png')
